...
class PlayerQualityButton extends StatelessWidget {
  const PlayerQualityButton({required this.contentUuid, required this.enabled, super.key});
        ...
        return PopupMenuButton<HlsVariant?>(
          ...
          itemBuilder: (context) => <PopupMenuEntry<HlsVariant?>>[
            ...
            for (final variant in state.variants)
              PopupMenuItem<HlsVariant?>(value: variant, child: Text(variant.label)),
          ],
          child: _Label(
...
class PlayerRateButton extends StatelessWidget {
  const PlayerRateButton({super.key});
  ...
  Widget build(BuildContext context) {
    ...
    return ListenableBuilder(
      ...
      builder: (context, _) => PopupMenuButton<double>(
        ...
        initialValue: controller.state.rate,
        ...
        child: _Label(
          text: context.l10n.playerSpeedValue($formatRate(controller.state.rate)),
